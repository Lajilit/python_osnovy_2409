...
with open('l5t3_text.txt', 'r', encoding='UTF-8') as surname_salary:
    number_of_stuff = 0
    sum_salary = 0
    for line in surname_salary:
        ...
        try:
            ...
            print(f'Salary {surname} is wrong: {salary}')
            while type(salary) is not int:
                try:
                    salary = int(input('Enter correct salary: '))
                except ValueError:
                    print(f'Salary {surname} is wrong: {salary}')
        finally:
            number_of_stuff += 1
